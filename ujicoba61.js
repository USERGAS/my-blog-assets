...
// ============================================================
    // LOGIKA BLASTER (HENSHIN) - WEB AUDIO API & VISUAL FX
    // ============================================================

    // 1. Inisialisasi Audio Engine ke RAM (Untuk Seamless Loop)
    const audioCtx = new (window.AudioContext || window.webkitAudioContext)();
    const standbyAudioData = { white: null, black: null, master: null };
    let currentStandbySource = null;

    // 2. Fungsi Load File OGG ke RAM
    ...
    // 3. Panggil Load (Otomatis jalan di latar)
    ...
    // 4. Play Seamless Tingkat Hardware
    const playSeamlessLoop = (key, volume = 0.5) => {
        if (audioCtx.state === 'suspended') audioCtx.resume();
        stopAllStandby(); 
        
        if (!standbyAudioData[key]) return; 
    ...
    // 6. Evaluasi Kombinasi Suara
    const evaluateStandbySound = () => {
        const w = state.blasterWhiteActive;
        const b = state.blasterBlackActive;

        if (w && b) {
            playSeamlessLoop('master', 0.6);
        } else if (w && !b) {
            playSeamlessLoop('white', 0.5);
        } else if (!w && b) {
            playSeamlessLoop('black', 0.5);
        }
    };

    // 7. Reset Visual dan State
    const resetBlaster = () => {
    ...
    // 8. Handler Utama Saat Tombol Ditekan
    const handleBlasterAction = (action) => {
        if (action === 'white') {
            ...
            // EFEK SENTAKAN (RECOIL) KE GAMBAR SENJATA
            const gunImg = document.getElementById("blaster-img");
            if (gunImg) {
                gunImg.classList.remove('recoil-effect'); 
                void gunImg.offsetWidth; // Trik ajaib restart animasi CSS
                gunImg.classList.add('recoil-effect');
            }
            ...
            if (!w && !b) {
                ...
                // COMBO TRIGGER (Bunyi Tring dulu, delay 300ms baru Tembak)
                playTring();
                ...
                if (w && !b) {
                    ...
                    if (state.userData.unlockedCharacters.includes('master_form')) {
                        if(DOM.blasterComboText) DOM.blasterComboText.textContent = "MASTER FORM!!!";
                        setTimeout(() => {
            ...
            // Kembalikan ke awal (Reset visual & audio) setelah nembak
            setTimeout(resetBlaster, 2000);
        }

        // Update Text Tampilan Sementara
        if (action !== 'trigger' && DOM.blasterComboText) {
