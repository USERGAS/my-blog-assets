...
// ================== LOGIKA BLASTER (HENSHIN) - FIXED AUDIO ==================
    ...
    const handleBlasterAction = (action) => {
        playTring(); // Selalu bunyi tring setiap klik apapun

        if (action === 'white') {
            if (!state.blasterWhiteActive) {
                state.blasterWhiteActive = true;
                evaluateStandbySound(); // Evaluasi suara standby
            }
        } 
            ...
            // Kembalikan ke awal setelah nembak (Delay 2 detik biar suara nembak kelar)
            setTimeout(resetBlaster, 2000);
        }

        // Update Text Layar Tampilan Combo
        if (action !== 'trigger') {
