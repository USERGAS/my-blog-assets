    ...
    const syncUserDataToServer = () => { 
        try { 
            if (state.userData && state.userData.lastLoginDate) { 
                ...
                // === BAGIAN DIAGNOSA HP ===
                // Ambil data langsung dari otak script (State)
                const cekSkin = state.userData.activeCharacter;
                const cekList = JSON.stringify(state.userData.unlockedCharacters);

                // TAMPILKAN POP-UP DI LAYAR HP (Hanya untuk ngecek)
                // Nanti kalau sudah benar, baris alert ini bisa dihapus/komentar
                alert(`🔍 CEK DATA DIKIRIM:\n\nSkin Aktif: ${cekSkin}\nList Skin: ${cekList}`);
                ...
                const url = `${CONFIG.APPS_SCRIPT_URL}?action=sync_user_data` +
                            ...
                            `&unlockedChars=${encodeURIComponent(cekList)}` + 
                            `&activeChar=${encodeURIComponent(cekSkin)}`; // Pakai variabel cekSkin

                fetch(url).then(response => response.json()).then(data => { 
                    if (data.status === 'error') { 
                        alert("❌ Gagal Kirim ke Google: " + data.message);
                    } else {
                        // alert("✅ Sukses Masuk Database!"); // Opsional
                    }
                }).catch(err => { 
                    alert("❌ Error Koneksi: " + err.toString());
                }); 
            } 
        } catch (err) { 
            alert("❌ Error Fatal JS: " + err.toString());
        } 
    };
