...
const attachModelToRunner = () => {
    ...
    // ============================================================
    // FUNGSI SYNC DATA (FIXED: PEMBACAAN VARIABEL STATE)
    // ============================================================
    const syncUserDataToServer = () => { 
        try { 
            if (state.userData && state.userData.lastLoginDate) { 
                ...
                // --- [BAGIAN KRUSIAL: PERBAIKAN BACA DATA] ---
                
                // 1. Ambil dari 'state.userData.activeCharacter' (Nama Panjang sesuai inisialisasi)
                // Hati-hati: Jangan pakai .activeChar karena di state namanya .activeCharacter
                const realActiveChar = state.userData.activeCharacter || 'kaitou';
                
                // 2. Ambil List Karakter
                const realUnlockedList = state.userData.unlockedCharacters || ['kaitou'];
                const unlockedStr = JSON.stringify(realUnlockedList);

                // --- [CCTV: CEK CONSOLE BROWSER (F12)] ---
                console.log("🔄 SYNCING TO SERVER...");
                console.log("👉 Skin yang dikirim:", realActiveChar);
                console.log("👉 List yang dikirim:", unlockedStr);

                // 3. Masukkan ke URL (Kirim sebagai parameter 'activeChar' agar ditangkap GAS)
                const url = `${CONFIG.APPS_SCRIPT_URL}?action=sync_user_data` +
                            ...
                            `&activeChar=${encodeURIComponent(realActiveChar)}`; // <--- INI KUNCINYA
                ...
                fetch(url).then(response => response.json()).then(data => { 
                    ...
                    console.error(`❌ Gagal koneksi sync: ${err.toString()}`); 
                }); 
            } 
        } catch (err) { 
            console.error(`❌ Error Fatal di JS: ${err.toString()}`); 
        } 
    };
...
// ================== LOGIKA SISTEM AKUN (RECOVERY) ==================
    ...
    // 2. Fungsi Load Data dari Server (Restore)
    const restoreAccount = async () => {
        ...
        // Konfirmasi agar tidak sengaja menimpa
        ...
        try {
            ...
            if (data.status === 'success') {
                ...
                const restoredData = {
                    ...
                    // Handle Unlocked Characters (Parse dari JSON String jika perlu)
                    ...
                    // Data misi & waktu kita reset ke hari ini biar aman
                ...
                alert(`Berhasil! Selamat datang kembali, ${data.username}! Halaman akan dimuat ulang.`);
                
                // Refresh Halaman Wajib
                location.reload(); 
            } else {
