document.addEventListener("DOMContentLoaded", () => {
    ...
    // [FITUR BARU] Helper untuk getar
    const triggerVibration = (pattern) => {
        // Cek apakah browser support vibration API (biasanya Android)
...
Ada yang mau ditanyakan? Atau mau langsung mulai berburu misi?`}]} 
    ...
    const checkMissionCompletion = (userInput = null, actionType = null) => { 
        ...
        if (missionAccomplished) { 
            ...
            // [FITUR BARU] Getar saat misi selesai (Getar - Diam - Getar)
    ...
    const dealDamageToMonster = (damage, animation) => {
        if ("none" !== state.isGameTurn) {
            ...
            // [FITUR BARU] Getar singkat saat memukul monster (Feedback pukulan)
            triggerVibration(50);
    ...
    const dealDamageToPlayer = (damage) => {
        if ("none" !== state.isGameTurn) {
            ...
            if (state.isPlayerDodging) {
                ...
                // [FITUR BARU] Getar agak lama saat player kena damage (Efek sakit)
    ...
    const setupEventListeners = () => {
        ...
        DOM.battleActionButtons.addEventListener("click", e => {
            if ("player" === state.isGameTurn && e.target.matches(".action-btn")) {
                ...
                // [TAMBAHAN] Getar ringan saat klik tombol apapun
                triggerVibration(10);
