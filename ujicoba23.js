...
// --- 2. PROSES GANTI KARAKTER (DENGAN INDIKATOR MB & %) ---
    const switchCharacter = (character) => {
        // Tutup Hub
        DOM.hubPopup.style.display = 'none';
        
        // Cek jika model sama (Lewati download)
        ...
        // PROSES DOWNLOAD MANUAL (XHR)
        const xhr = new XMLHttpRequest();
        xhr.open("GET", character.modelSrc, true);
        xhr.responseType = "blob"; // Penting: Kita download sebagai data Blob

        // 1. Event saat proses download berjalan (Progress)
        xhr.onprogress = (event) => {
            if (event.lengthComputable) {
                ...
                // Hitung MB
                const loadedMB = (event.loaded / (1024 * 1024)).toFixed(2); // 2 desimal
                ...
                // Jika server tidak kasih info total file (jarang terjadi)
                const loadedMB = (event.loaded / (1024 * 1024)).toFixed(2);
                DOM.skinProgressDetails.textContent = `Mengunduh... (${loadedMB} MB)`;
            }
        };

        // 2. Event saat download selesai
        xhr.onload = () => {
            if (xhr.status === 200) {
                // Buat URL lokal dari data yang sudah di-download
                ...
                }, 500); // Delay dikit biar user lihat 100%
            } else {
                handleError();
            }
        };
...
// --- MASTER SWITCH (VERSI TOMBOL ANIMASI TETAP ADA) ---
    const applyCharacterMode = (charId) => {
        ...
        // 3. LOGIKA SEMBUNYIKAN / TAMPILKAN
        if (charId !== 'kaitou') {
            // === MODE GAME (Rider) ===
            state.isGameMode = true; 
            
            // Sembunyikan Input Chat (Ketik Pesan) -> INI TETAP HILANG
            if(DOM.chatInputArea) DOM.chatInputArea.style.setProperty('display', 'none', 'important');
            
            // Tampilkan Tombol Animasi -> INI SEKARANG MUNCUL
