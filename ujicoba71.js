...
// --- MASTER SWITCH (FIXED: Sinkronisasi Kuat) ---
    const applyCharacterMode = (charId, skipModelUpdate = false) => {
        ...
        // 3. Update Model Utama (Jika tidak di-skip oleh switchCharacter)
        // Ini berjalan saat Refresh halaman / Load awal
        if (!skipModelUpdate) {
            const currentSrc = DOM.modelViewer.getAttribute('src');
            // Jika src kosong atau beda URL, ganti.
            // Note: Kita biarkan saja jika src berupa blob (karena itu hasil switchCharacter)
            if (!currentSrc || (currentSrc !== char.modelSrc && !currentSrc.startsWith('blob:'))) {
                DOM.modelViewer.src = char.modelSrc;
            }
        }

        // 4. Update Model Battle (Selalu Copy dari Model Utama)
        // [FIX] Kita ambil src langsung dari properti src, bukan getAttribute, biar dapat full path/blob
