document.addEventListener("DOMContentLoaded", () => {
    ...
    const attachModelToRunner = () => {
        ...
        // SETTING KHUSUS GAME RUNNER
        DOM.modelViewer.style.visibility = "visible";
        DOM.modelViewer.style.backgroundColor = "transparent"; // Hapus background abu-abu
        
        // 1. KUNCI KAMERA LIHAT BELAKANG (180deg) & AGAK ATAS (75deg)
        DOM.modelViewer.cameraOrbit = "180deg 75deg 3m"; 
        DOM.modelViewer.fieldOfView = "30deg"; // Zoom in dikit biar dramatis
        
        // 2. MATIKAN KONTROL PLAYER (Biar ga bisa diputer2 pas main)
        DOM.modelViewer.removeAttribute('camera-controls'); 
        
        // 3. PAKSA ANIMASI LARI
        if (DOM.modelViewer.availableAnimations.includes("Run")) {
...
// ================== GAME RUNNER (FIXED ANIMATION & FX) ==================
    let touchStartX = 0;
    let touchEndX = 0;
    ...
    const startRunnerGame = () => {
        ...
        state.runnerCoins = 0;
        state.runnerLives = 3;
        state.runnerSpeed = 2500; // SEDIKIT DIPERLAMBAT BIAR LEBIH FAIR (Start Jauh)
        
        DOM.runnerOverlay.style.display = 'block';
        DOM.runnerMsgOverlay.style.display = 'flex';
        DOM.runnerMsgTitle.textContent = "CYBER RUN";
        DOM.runnerMsgDesc.textContent = "Geser Layar (Swipe) Kiri/Kanan!";
    ...
    const runGameLoop = () => {
        ...
        const spawnObject = () => {
            ...
            // Durasi animasi disamakan dengan runnerSpeed agar sinkron
            ...
            // Hitbox Check (Di 85% animasi)
            setTimeout(() => {
                if(!state.isRunnerActive || !obj.parentNode) return;
                
                if (state.runnerLane === lane) {
                    if (isCoin) {
                        collectCoinRunner(obj, lane);
                    } else {
                        hitObstacleRunner(obj);
                    }
                }
            }, state.runnerSpeed * 0.85); 

            setTimeout(() => { if(obj.parentNode) obj.remove(); }, state.runnerSpeed);
            
            // Spawn rate sedikit dipercepat
            state.runnerTimer = setTimeout(spawnObject, 1500);
        };
        
        spawnObject();
        
        // [FIX ANIMASI DIAM] Safety check loop
        // Memastikan animasi lari tidak berhenti di tengah jalan
        const animCheck = setInterval(() => {
            if(!state.isRunnerActive) { clearInterval(animCheck); return; }
            if(DOM.modelViewer.paused) DOM.modelViewer.play();
        }, 1000);
    };
    ...
    const attachModelToRunner = () => {
        ...
        // [FIX ANIMASI] Paksa mainkan 'Run'
        setTimeout(() => {
            DOM.modelViewer.animationName = "Run";
            DOM.modelViewer.timeScale = 1; // Kecepatan normal
            DOM.modelViewer.play();
        }, 100); // Delay dikit biar transisi mulus

        updatePlayerLane();
    };

    const moveRunner = (direction) => {
        if(direction === 'left' && state.runnerLane > 0) state.runnerLane--;
        if(direction === 'right' && state.runnerLane < 2) state.runnerLane++;
        updatePlayerLane();
    };
    ...
    const showRunnerFeedback = (type, lane) => {
        // Efek Pop Up +1
        if (type === 'coin') {
            ...
            popup.textContent = "+1 🪙";
            const positions = ['20%', '50%', '80%'];
            popup.style.left = positions[lane];
            popup.style.bottom = "150px"; // Muncul di atas player
            DOM.runnerOverlay.appendChild(popup);
    ...
    const collectCoinRunner = (obj, lane) => {
        obj.remove();
        state.runnerCoins++;
        DOM.runnerCoinCount.textContent = state.runnerCoins;
        
        showRunnerFeedback('coin', lane); // Tampilkan efek visual
    ...
    const hitObstacleRunner = (obj) => {
        ...
        showRunnerFeedback('hit'); // Tampilkan efek visual
    ...
    // ... (updateRunnerLivesUI, endRunnerGame, resetModelPosition, setupRunnerControls SAMA seperti sebelumnya) ...
    // ... Pastikan fungsi-fungsi itu tetap ada ya!
    // Biar aman saya tulis ulang fungsi penting sisanya di sini:
    ...
    const setupRunnerControls = () => {
        ...
        runnerContainer.addEventListener('touchstart', (e) => { touchStartX = e.changedTouches[0].screenX; }, {passive: false});
        runnerContainer.addEventListener('touchend', (e) => { touchEndX = e.changedTouches[0].screenX; handleSwipe(); }, {passive: false});
        runnerContainer.addEventListener('touchmove', (e) => { if(state.isRunnerActive) e.preventDefault(); }, {passive: false});
    };
