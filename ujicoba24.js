...
// --- PROSES GANTI KARAKTER (FIX: BLOB HANDLING) ---
    const switchCharacter = (character) => {
        DOM.hubPopup.style.display = 'none';
        
        // Cek Cepat: Jika src masih sama (dan bukan blob), skip
        ...
        xhr.onload = () => {
            if (xhr.status === 200) {
                // 1. Buat URL Objek dari file yang didownload
                const blob = xhr.response;
                const objectURL = URL.createObjectURL(blob);
                
                // 2. TEMPEL KE SEMUA VIEWER (Utama & Battle)
                DOM.modelViewer.src = objectURL;
                if(DOM.battleModelViewer) DOM.battleModelViewer.src = objectURL;
                
                // 3. Simpan State
                state.userData.activeCharacter = character.id;
                saveUserData();
                
                // 4. Update UI TAPI JANGAN RESET MODEL (Parameter false)
                applyCharacterMode(character.id, false); 
                
                // 5. Animasi & Notifikasi
                playAnimation(DOM.modelViewer, CONFIG.ANIMATION_NAME_IDLE);
                addMessageToChat('bot', `🔄 Kostum berhasil diganti: **${character.name}**!`);

                setTimeout(() => { DOM.skinLoadingOverlay.style.display = 'none'; }, 500);
            } else {
                handleError();
            }
        };
...
// --- MASTER SWITCH (FIX: PARAMETER UPDATE MODEL) ---
    const applyCharacterMode = (charId, updateModel = true) => {
        const char = CONFIG.CHARACTERS.find(c => c.id === charId);
        if (!char) return;
        ...
        // 4. Update MODEL UTAMA (Hanya jika diminta)
        if (updateModel) {
            const currentSrc = DOM.modelViewer.getAttribute('src');
            // Cek agar tidak reload jika URL-nya sama persis
            if (currentSrc !== char.modelSrc) {
                DOM.modelViewer.src = char.modelSrc;
            }
            
            // Update MODEL BATTLE sekalian (jika mode update aktif)
