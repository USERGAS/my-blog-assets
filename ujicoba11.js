...
// ================== GAME RUNNER (SWIPE EDITION) ==================
    // Variabel Global untuk Swipe
    let touchStartX = 0;
    let touchEndX = 0;

    const startRunnerGame = () => {
        state.isRunnerActive = true;
        state.runnerLane = 1; // 0:Kiri, 1:Tengah, 2:Kanan
        state.runnerCoins = 0;
        state.runnerLives = 3;
        state.runnerSpeed = 2000; // Kecepatan awal
    ...
    const runGameLoop = () => {
        ...
        const spawnObject = () => {
            ...
            // Cek Tabrakan di 85% perjalanan animasi (saat objek di depan player)
            setTimeout(() => {
                if(!state.isRunnerActive || !obj.parentNode) return;
                
                // Jika Jalur Player == Jalur Objek
            ...
            // Hapus objek jika sudah lewat
            setTimeout(() => { if(obj.parentNode) obj.remove(); }, state.runnerSpeed);
            
            // Spawn objek berikutnya (sedikit lebih cepat seiring waktu kalau mau dikembangkan)
            state.runnerTimer = setTimeout(spawnObject, 1300); 
        };
    ...
    const updatePlayerLane = () => {
        const wrapper = DOM.runnerOverlay.querySelector('.runner-player-wrapper');
        if(wrapper) {
            // [FIX POSISI PLAYER] Sama persis dengan posisi objek
    ...
    const collectCoinRunner = (obj) => {
        ...
        triggerVibration(50); // Getar enak

        // [FIX WIN CONDITION] Menang langsung jika koin >= 10
        if(state.runnerCoins >= CONFIG.COINS_PER_RUN) {
            endRunnerGame(true); // TRUE = Menang
        }
    };
    ...
    const hitObstacleRunner = (obj) => {
        ...
        triggerVibration(300); // Getar kasar
        ...
        if(state.runnerLives <= 0) {
            endRunnerGame(false); // FALSE = Kalah
        }
    };
    ...
    const endRunnerGame = (isWin) => {
        ...
        if (isWin) {
            ...
            triggerVibration([100, 50, 100]); // Getar Menang
        } else {
            addMessageToChat('bot', `💥 Awas nabrak! Misi gagal. Koin tidak terselamatkan.`);
        }
    };

    // Fungsi Baru: Setup Kontrol (Swipe & Keyboard)
    ...
    const handleSwipe = () => {
        if (!state.isRunnerActive) return;
        const threshold = 30; // Sensitivitas geser
