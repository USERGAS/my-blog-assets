document.addEventListener("DOMContentLoaded", () => {
    ...
    const CONFIG = {
        ...
        FULL_MISSIONS_LIST: [
            ...
            { id: 'crack_safe', text: `Bobol brankas (Titik Kuning) di Peta` },
    ...
    const DOM = {
        ...
        sfxSafeTick: document.getElementById("sfx-safe-tick"), // Suara putaran biasa
        sfxSafeClick: document.getElementById("sfx-safe-click"), // Suara sweet spot
    ...
    let state = {
        ...
        currentDialAngle: 0,
        lastTickAngle: 0, // Untuk tracking suara tik
    ...
    // [AUDIO PRELOADER] Agar suara brankas ready
    const preloadGameAudios = () => {
        const audios = [DOM.sfxSafeTick, DOM.sfxSafeClick, DOM.sfxSafeOpen, DOM.sfxAttack, DOM.sfxMonsterHit];
        audios.forEach(audio => {
            if(audio) {
                audio.load(); // Memaksa browser meload data
                audio.volume = 0.8; 
            }
        });
    };
    ...
    const toggleChatWindow = () => {
        ...
        if (isVisible) {
            ...
            // [OPTIMASI] Load audio saat chat dibuka
            preloadGameAudios();

            if (DOM.battleOverlay.style.display === 'flex' || state.isSafeGameActive) {
    ...
    const syncUserDataToServer = () => { 
        try { 
            if (state.userData && state.userData.lastLoginDate) { 
                ...
                const data = { 
                    userId: userId, 
                    username: state.userData.username || "Anonim", 
                ...
                fetch(url).then(response => response.json()).then(data => { 
                    if (data.status === 'error') console.error(`Error dari Google Script: ${data.message}`); 
                }).catch(err => console.error(`Gagal terhubung ke server sync: ${err.toString()}`)); 
            } 
        } catch (err) { console.error(`Error di dalam fungsi syncUserDataToServer: ${err.toString()}`); } 
    };
    ...
    const saveUserData = () => { 
        ...
        } catch (e) { console.error("Gagal menyimpan data ke localStorage:", e); } 
    };

    const updateCoinDisplay = () => { 
        if (DOM.coinAmountElem) DOM.coinAmountElem.textContent = state.userData.coins || 0; 
    };
...
*   **👾 Lawan Monster (Titik Merah):** Waspada! Radar di pojok bisa mendeteksi monster. Kalahkan untuk dapat koin.
*   **🔐 Bobol Brankas (Titik Kuning):** Temukan kode rahasia dengan memutar dial dan dengarkan suaranya!
...
Ada yang mau ditanyakan? Atau mau langsung mulai berburu misi?`}]} 
    ...
    const fetchUserRankAndLeaderboard = async () => { 
        try { 
            ...
            const myUsername = state.userData.username; 
            const myRankIndex = data.leaderboard.findIndex(p => p.username === myUsername); 
            const myRank = (myRankIndex !== -1) ? myRankIndex + 1 : '10+'; 
            updateRankDisplay(myRank); 
        } catch (error) { 
            console.error("Gagal mengambil peringkat:", error); 
            DOM.rankDisplay.classList.remove('visible'); 
        } 
    };
    ...
    const updateRankDisplay = (rank) => { 
        ...
        DOM.rankDisplay.classList.add('visible'); 
    };
        ...
        try { 
            if (state.leaderboardData[type].length === 0) { 
                ...
                if (data.status === 'success') { 
                    state.leaderboardData[type] = data.leaderboard; 
                } else { 
            ...
            if (leaderboard.length === 0) { 
                DOM.leaderboardListContainer.innerHTML = `<p id="leaderboard-message">Belum ada data. Jadilah yang pertama!</p>`; 
                return; 
            } 
            
            const list = document.createElement('ol'); 
            list.id = 'full-leaderboard-list'; 
            ...
            DOM.leaderboardListContainer.innerHTML = ''; 
            DOM.leaderboardListContainer.appendChild(list); 
        } catch (error) { 
            DOM.leaderboardListContainer.innerHTML = `<p id="leaderboard-message">Gagal memuat papan skor. Coba lagi nanti.</p>`; 
    ...
    const checkMissionCompletion = (userInput = null, actionType = null) => { 
        ...
        if (missionAccomplished) { 
            ...
            const allMissionsNowComplete = state.userData.dailyMissions.every(m => m.completed); 
            if (allMissionsNowComplete) { 
                missionMessage = `🏆 Hebat! Semua misi harian telah selesai!`; 
            } 
    ...
    const updateMapAlert = () => { 
        ...
        const totalDots = DOM.missionMapContainer.querySelectorAll('.monster-dot, .yellow-dot').length; 
    ...
    const spawnMonsters = () => { 
        ...
        if(Math.random() > 0.5) {
             ...
             yellowDot.addEventListener('click', (e) => {
                 e.stopPropagation();
                 state.activeMonsterDot = yellowDot; 
                 startSafeGame();
             });
    ...
    // ================== LOGIKA BRANKAS (SAFE CRACKING) ==================
    const startSafeGame = () => {
        state.isSafeGameActive = true;
        state.safeCurrentLevel = 0;
        state.currentDialAngle = 0;
        state.lastTickAngle = 0; // Reset tick sound
        state.safeCombination = [];
        for(let i=0; i<3; i++) {
            state.safeCombination.push(Math.floor(Math.random() * 360));
        }
        
        // Memastikan audio terload sebelum mulai
        preloadGameAudios();
        
        DOM.safeDial.style.transform = `rotate(0deg)`;
        DOM.safeLights.forEach(light => light.classList.remove('unlocked'));
        DOM.modelViewer.style.visibility = "hidden";
        DOM.safeOverlay.style.display = 'flex';
        
        addMessageToChat('bot', '🕵️ Kamu menemukan brankas terkunci! Naikkan Volume 🔊, putar dial perlahan, dan tahan saat bunyi "KLIK" beda!');
    };

    const checkSafeSweetSpot = () => {
        if (state.safeCurrentLevel >= 3) return;
        
        const target = state.safeCombination[state.safeCurrentLevel];
        const diff = Math.abs(state.currentDialAngle - target);
        
        // Sweet Spot Zone
        if (diff < 8 || (360 - diff) < 8) {
            if (!state.safeUnlockTimer) {
                // Bunyi Sweet Spot (KLIK BESAR)
                if(DOM.sfxSafeClick) {
                    DOM.sfxSafeClick.currentTime = 0;
                    DOM.sfxSafeClick.play().catch(e => {});
                }
                triggerVibration(50); // Getar tajam
                
                state.safeUnlockTimer = setTimeout(() => {
                    unlockSafeLevel();
                }, 800); 
            }
        } else {
    ...
    const unlockSafeLevel = () => {
        ...
        if (state.safeCurrentLevel >= 3) {
            setTimeout(endSafeGameWin, 500);
        }
    };
    ...
    const endSafeGameWin = () => {
        ...
        state.userData.coins += CONFIG.COINS_PER_SAFE;
        updateCoinDisplay();
        saveUserData();
        checkMissionCompletion(null, 'crack_safe');
        
        addMessageToChat('bot', `🔓 KLIK! Brankas terbuka! Kamu menemukan ${CONFIG.COINS_PER_SAFE} Koin di dalamnya!`);
        updateMapAlert();
    };
    ...
    const handleDialInput = (e, isEnd = false) => {
        if (isEnd) {
            state.isDraggingDial = false;
            return;
        }
        
        const rect = DOM.safeDial.getBoundingClientRect();
        ...
        state.currentDialAngle = degrees;
        DOM.safeDial.style.transform = `rotate(${degrees}deg)`;
        
        // [AUDIO] Logic suara Tik setiap 15 derajat
        if (Math.abs(degrees - state.lastTickAngle) > 15) {
            if(DOM.sfxSafeTick) {
                DOM.sfxSafeTick.currentTime = 0; // Reset biar bisa play cepat
                DOM.sfxSafeTick.play().catch(e => {}); // Play suara tik
            }
            // Update last angle, handling wrap around 360 is simplified here
            state.lastTickAngle = degrees;
            triggerVibration(5); // Getar sangat halus
        }
    ...
    const endBattle = (isWin) => {
        ...
        if (isWin) {
            ...
            const prize = CONFIG.COINS_PER_MONSTER;
            ...
            addMessageToChat("bot", `🎉 KAMU MENANG! Monster berhasil dikalahkan dan kamu mendapatkan ${prize} Koin!`);
        } else {
    ...
    const addBattleLog = (text) => {
        ...
        p.textContent = `> ${text}`;
        DOM.battleLogArea.appendChild(p);
    ...
    const populateShopList = () => {
        ...
        CONFIG.SHOP_ITEMS.forEach(item => {
            ...
            if (item.discountCost && now <= discountEndDate) {
                isDiscountActive = true;
            }

            let priceHTML = `<span class="item-price">${item.cost} Koin</span>`;
            if (isDiscountActive) {
                priceHTML = `<span class="item-price" style="color: #4caf50; font-weight: bold;">
                               <del style="color: #999;">${item.cost} Koin</del> ${item.discountCost} Koin (EVENT TERBATAS)
                             </span>`;
            }
    ...
    const populateHistoryList = () => {
        ...
        if (history.length === 0) {
            ...
            history.reverse().forEach(h => {
                const li = document.createElement("li");
                const dateStr = new Date(h.timestamp).toLocaleString("id-ID", { day: "numeric", month: "short", year: "numeric", hour: "2-digit", minute: "2-digit" });
                li.innerHTML = `<div class="item-details"><span class="item-name">Kode: <code class="item-code">${h.code}</code></span><span class="item-timestamp">Ditukar pada: ${dateStr}</span></div><button class="copy-btn" data-code="${h.code}">Salin</button>`;
    ...
    const setupEventListeners = () => {
        ...
        DOM.fullscreenBtn.addEventListener("click", () => { 
            if (!document.fullscreenElement) { 
                DOM.chatWindow.requestFullscreen(); 
                checkMissionCompletion(null, 'use_fullscreen'); 
            } else { 
        ...
        DOM.toast.addEventListener('click', (e) => { 
            if (e.target !== DOM.toastClose) { 
                DOM.toast.classList.remove('visible'); 
                toggleChatWindow(); 
            } 
        });
        DOM.toastClose.addEventListener('click', (e) => { 
            e.stopPropagation(); 
            DOM.toast.classList.remove('visible'); 
        });
        ...
        DOM.missionToggleBtn.addEventListener("click", (e) => { 
            e.stopPropagation(); 
            const isVisible = DOM.missionPanel.classList.contains('visible'); 
            closeAllPanels(); 
            if (!isVisible) DOM.missionPanel.classList.add('visible'); 
        });
        
        DOM.animationToggleBtn.addEventListener("click", (e) => { 
            e.stopPropagation(); 
            const isVisible = DOM.animationControls.classList.contains('visible'); 
            closeAllPanels(); 
            if (!isVisible) DOM.animationControls.classList.add('visible'); 
        });
        
        DOM.closeHubBtn.addEventListener('click', () => { DOM.hubPopup.style.display = 'none'; });
        
        DOM.hubTabs.addEventListener('click', e => { 
            if (e.target.matches('.hub-tab-btn')) { 
        ...
        DOM.shopList.addEventListener('click', async (e) => {
            ...
            const now = new Date();
            const discountEndDate = CONFIG.DISCOUNT_END_DATE ? new Date(CONFIG.DISCOUNT_END_DATE) : new Date(0);
            let currentPrice = item.cost;

            if (now <= discountEndDate && item.discountCost) {
                currentPrice = item.discountCost;
            }

            if (state.userData.coins < currentPrice) {
                alert("Koin kamu tidak cukup!");
                return;
            }

            const confirmed = confirm(`Anda yakin ingin menukarkan ${currentPrice} koin dengan "${item.name}"?`);
            if (!confirmed) return;
        ...
        DOM.battleActionButtons.addEventListener("click", e => {
            if ("player" === state.isGameTurn && e.target.matches(".action-btn")) {
                ...
                if (action === "punch") dealDamageToMonster(12, "PunchC");
                    ...
                    addBattleLog("Kamu bersiap untuk menghindar!");
                    switchTurn("enemy");
                } else if (action === "special" && state.superMeter >= CONFIG.SUPER_METER_MAX) {
                    dealDamageToMonster(50, "Twirl");
                    state.superMeter = 0;
                }
        ...
        DOM.saveUsernameBtn.addEventListener('click', () => { 
            ...
            if (username.length >= 3 && username.length <= 15 && /^[a-zA-Z0-9_ ]+$/.test(username)) { 
                ...
                DOM.usernamePopup.style.display = 'none'; 
                finishInitialization(); 
                setTimeout(() => { 
                    if (!DOM.chatWindow.classList.contains('visible')) toggleChatWindow(); 
                }, 300); 
            } else { 
                alert('Nama tidak valid! Gunakan 3-15 karakter (huruf, angka, spasi, _).'); 
            } 
        });
        ...
        DOM.rankDisplay.addEventListener('click', () => { 
            const activeType = DOM.leaderboardTypeSelector.querySelector('.active').dataset.leaderboardType; 
            displayFullLeaderboard(activeType); 
        });
        ...
        DOM.leaderboardTypeSelector.addEventListener('click', (e) => { 
            if (e.target.matches('.leaderboard-type-btn')) { 
                const type = e.target.dataset.leaderboardType; 
                displayFullLeaderboard(type); 
            } 
        });

        // Event Listener khusus Safe Game
        if(DOM.closeSafeBtn) {
            DOM.closeSafeBtn.addEventListener('click', closeSafeGame);
        }
        
        // Mouse & Touch Events untuk Dial Brankas
        if(DOM.safeDial) {
            DOM.safeDial.addEventListener('mousedown', (e) => {
                state.isDraggingDial = true;
                handleDialInput(e);
            });
            window.addEventListener('mousemove', (e) => {
                if(state.isDraggingDial) handleDialInput(e);
            });
            window.addEventListener('mouseup', () => handleDialInput(null, true));

            DOM.safeDial.addEventListener('touchstart', (e) => {
                state.isDraggingDial = true;
                handleDialInput(e);
            }, {passive: false});
            window.addEventListener('touchmove', (e) => {
                if(state.isDraggingDial) {
                    e.preventDefault(); // Mencegah scroll saat muter dial
                    handleDialInput(e);
                }
            }, {passive: false});
    ...
    const initialize = () => {
        ...
        setInterval(() => { 
            ...
            const hasMonsters = DOM.missionMapContainer.querySelectorAll(".monster-dot, .yellow-dot").length > 0; 
            if (!isBattle && !isSafeGame && !hasMonsters) { 
                const now = Date.now(); 
