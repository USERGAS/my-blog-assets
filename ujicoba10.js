...
*   **🔑 Tukar Hadiah:** Kumpulkan koinmu dan tukar dengan **Kode Akses Link Download Tanpa Iklan** di User Hub (ikon koin 🪙).`}]} 
    ...
    const runGameLoop = () => {
        ...
        const spawnObject = () => {
            ...
            const laneOffset = (lane - 1) * 60; 
            obj.style.marginLeft = `${laneOffset}px`;
            ...
            const nextTime = Math.max(800, state.runnerSpeed * 0.7); 
    ...
    const endRunnerGame = () => {
        ...
        state.userData.coins += state.runnerCoins;
        updateCoinDisplay();
        saveUserData();
        ...
        addMessageToChat('bot', `🏃‍♂️ Cyber Run Selesai! Kamu berhasil mengamankan ${state.runnerCoins} Koin!`);
    };
    ...
    const setupEventListeners = () => {
        ...
        DOM.runLeftBtn.addEventListener('click', () => moveRunner('left'));
        DOM.runRightBtn.addEventListener('click', () => moveRunner('right'));
