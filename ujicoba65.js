...
// ============================================================
    ...
    const createErrorSparks = () => {
        ...
        for(let i = 0; i < 20; i++) {
            ...
            setTimeout(() => spark.remove(), 400); // Hapus elemen setelah animasi kelar
        }
    };

    // Fungsi Transisi Layar Henshin Seamless
    const playHenshinTransition = (targetCharId, gradientColors) => {
        // 1. Buat Layar Cahaya Meledak
        const flash = document.createElement('div');
        flash.className = 'henshin-flash-overlay';
        flash.style.background = `radial-gradient(circle, ${gradientColors[0]} 20%, ${gradientColors[1]} 80%)`;
        document.body.appendChild(flash);

        // 2. Tepat saat layar paling putih/terang (200ms), Sembunyikan Senjata & Ganti Model
        setTimeout(() => {
            DOM.blasterOverlay.style.display = 'none'; 
            switchCharacter(CONFIG.CHARACTERS.find(c => c.id === targetCharId));
        }, 200);

        // 3. Hapus elemen flash setelah memudar
