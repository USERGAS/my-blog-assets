document.addEventListener("DOMContentLoaded", () => {
    ...
    const CONFIG = {
        ...
        // Pastikan format tanggal ISO (YYYY-MM-DDTHH:mm:ss) agar kompatibel lintas browser
    ...
    const DOM = {
        ...
        hubPopupBox: document.getElementById("kaitou-hub-box"), // Fixed selector ID
    ...
    const addMessageToChat = (role, text, mediaInfo = null) => { 
        ...
        if (role === "bot") { 
            // Cek apakah sedang battle, jika iya JANGAN ganti animasi model utama
    ...
    const toggleChatWindow = () => {
        ...
        if (isVisible) {
            ...
            // [OPTIMASI] Cek Battle Status agar model tidak tumpuk
            if (DOM.battleOverlay.style.display === 'flex') {
            ...
            // Jangan panggil animasi jika tertutup, hemat resource
            playAnimation(DOM.modelViewer, CONFIG.ANIMATION_NAME_IDLE);
            closeAllPanels();
        }
    };
...
Ada yang mau ditanyakan? Atau mau langsung mulai berburu misi?`}]} 
    ...
    const startBattle = () => {
        ...
        // [OPTIMASI] Sembunyikan model utama agar tidak tumpuk
        DOM.modelViewer.style.visibility = "hidden";
    ...
    const endBattle = (isWin) => {
        ...
        // [OPTIMASI] Kembalikan visibility model utama jika chat window masih terbuka
    ...
    const switchTab = (tabName) => {
        ...
        // Pastikan kita memilih hanya elemen direct child dari hub content wrapper atau gunakan ID yang spesifik
        // Disini kita asumsikan ID patternnya 'hub-content-namaTab'
    ...
    // ================== LOGIKA TOKO & DISKON (DIOPTIMALKAN) ==================
    const populateShopList = () => {
        ...
        CONFIG.SHOP_ITEMS.forEach(item => {
            ...
            // [OPTIMASI] Cek Diskon yang lebih aman
            ...
            // Disable tombol jika koin tidak cukup (cek harga diskon jika aktif)
    ...
    const setupEventListeners = () => {
        ...
        // EVENT HANDLER TOKO (REDEEM) YANG DIPERBAIKI
        DOM.shopList.addEventListener('click', async (e) => {
            ...
            // [OPTIMASI] Hitung ulang harga saat klik untuk validasi ganda
            const now = new Date();
            const discountEndDate = CONFIG.DISCOUNT_END_DATE ? new Date(CONFIG.DISCOUNT_END_DATE) : new Date(0);
            ...
            // Validasi Saldo Client-Side Lagi
        ...
        DOM.battleActionButtons.addEventListener("click", e => {
            // [OPTIMASI] Pastikan target adalah tombol action
            if ("player" === state.isGameTurn && e.target.matches(".action-btn")) {
                const action = e.target.dataset.action;
                
                // Mencegah spam klik tombol saat animasi berjalan
                // Kita bisa mendisable tombol sebentar
                ...
                // Update UI akan meng-enable kembali tombol yang valid sesuai giliran
                // setTimeout kecil agar logika UI berjalan setelah flow ini selesai
        ...
        setInterval(() => { 
            ...
            if (!isBattle && !hasMonsters) { 
                ...
                if (now - (state.userData.lastMonsterSpawn || 0) > 1 * 60 * 1000) { 
                    addMessageToChat("bot", "🚨 Sinyal monster baru terdeteksi di peta!"); 
                    spawnMonsters(); 
                    state.userData.lastMonsterSpawn = now; 
                    saveUserData(); 
                } 
