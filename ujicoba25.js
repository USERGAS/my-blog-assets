...
// --- PROSES GANTI KARAKTER (FIXED: Integrasi dengan Master Switch) ---
    const switchCharacter = (character) => {
        ...
        xhr.onload = () => {
            if (xhr.status === 200) {
                ...
                // [FIX DI SINI] Panggil applyCharacterMode dengan parameter TRUE (Skip update model)
                // Karena modelnya BARU SAJA kita update di baris atas pakai objectURL
        ...
        const handleError = () => {
            DOM.skinLoadingOverlay.style.display = 'none';
            alert("Gagal mengunduh skin. Cek koneksi atau link model.");
        };

        xhr.send();
    };

// --- MASTER SWITCH (FIXED: Mencegah Bentrok dengan Loading Bar) ---
    const applyCharacterMode = (charId, skipModelUpdate = false) => {
        ...
        // 3. Logic Sembunyikan/Tampilkan UI
        if (charId !== 'kaitou') {
            state.isGameMode = true; 
            if(DOM.chatInputArea) DOM.chatInputArea.style.setProperty('display', 'none', 'important');
            if(DOM.animationToggleBtn) DOM.animationToggleBtn.style.display = 'block'; // Tombol animasi tetap ada
        } else {
        ...
        // 4. Update MODEL UTAMA (Hanya jika TIDAK diminta skip)
        if (!skipModelUpdate) {
            const currentSrc = DOM.modelViewer.getAttribute('src');
        ...
        // 5. Update MODEL BATTLE (Selalu sinkronkan dengan Model Utama)
        // Kita copy apapun source yang dipakai model utama (entah itu URL atau Blob)
        if (DOM.battleModelViewer) {
            const mainSrc = DOM.modelViewer.getAttribute('src');
            const battleSrc = DOM.battleModelViewer.getAttribute('src');
            
            if (battleSrc !== mainSrc) {
