...
// ============================================================
    // LOGIKA BLASTER: RESET & VISUAL EFFECTS (FULL VERSION)
    // ============================================================
    
    // Fungsi untuk mematikan semua efek saat reset
    const resetBlaster = () => {
        state.blasterWhiteActive = false;
        state.blasterBlackActive = false;
        stopAllStandby(); // Matikan suara
        
        if (DOM.blasterComboText) DOM.blasterComboText.textContent = "STANDBY";
        
        // MATIKAN SEMUA CAHAYA VISUAL (Hapus class CSS)
        if(DOM.btnWhite) {
            DOM.btnWhite.classList.remove('glow-white-active');
            DOM.btnWhite.classList.remove('critical-overload');
        }
        if(DOM.btnBlack) {
            DOM.btnBlack.classList.remove('glow-black-active');
            DOM.btnBlack.classList.remove('critical-overload');
        }
        if(DOM.btnTrigger) {
            DOM.btnTrigger.classList.remove('trigger-pulled');
        }
        
        // Hapus efek getar pada senjata
        const blasterContainer = document.getElementById('blaster-container');
        if (blasterContainer) blasterContainer.classList.remove('blaster-shake');
    };

    // Fungsi Utama Klik Senjata
    const handleBlasterAction = (action) => {
        if (action === 'white') {
            ...
            if (!state.blasterBlackActive) {
                ...
                // NYALAKAN CAHAYA MERAH
                if(DOM.btnBlack) DOM.btnBlack.classList.add('glow-black-active');
            }
        } 
        else if (action === 'trigger') {
            stopAllStandby(); 
            
            // Efek visual pelatuk ditarik kilat (150ms)
            if(DOM.btnTrigger) {
                DOM.btnTrigger.classList.add('trigger-pulled');
                setTimeout(() => DOM.btnTrigger.classList.remove('trigger-pulled'), 150);
            }
            ...
            if (!w && !b) {
                // ===============================
                // 1. NORMAL BLAST (Tembak Biasa)
                // ===============================
                if(DOM.blasterComboText) DOM.blasterComboText.textContent = "BLAST!";
                if(DOM.sfxBlastNormal) { 
                    DOM.sfxBlastNormal.currentTime = 0; 
                    DOM.sfxBlastNormal.play().catch(e=>{}); 
                }
                setTimeout(resetBlaster, 1000);
            } 
            else {
                // ===============================
                // COMBO (Selalu ada suara Tring dulu)
                // ===============================
                playTring();

                if (w && !b) {
                    // 2. FINISHER (Putih Saja)
                    if(DOM.blasterComboText) DOM.blasterComboText.textContent = "FINISHER!";
                    setTimeout(() => {
                        if(DOM.sfxBlastFinisher) { 
                            DOM.sfxBlastFinisher.currentTime = 0; 
                            DOM.sfxBlastFinisher.play().catch(e=>{}); 
                        }
                    }, 300);
                    setTimeout(resetBlaster, 2000);
                } 
                else if (!w && b) {
                    // 3. HENSHIN DEFAULT (Hitam Saja -> Kembali ke Kaitou)
                    if(DOM.blasterComboText) DOM.blasterComboText.textContent = "HENSHIN: DEFAULT";
                    setTimeout(() => {
                        if(DOM.sfxHenshinDefault) { 
                            DOM.sfxHenshinDefault.currentTime = 0; 
                            DOM.sfxHenshinDefault.play().catch(e=>{}); 
                        }
                    }, 300);
                    // Panggil fungsi ganti karakter
                    setTimeout(() => { switchCharacter(CONFIG.CHARACTERS.find(c => c.id === 'kaitou')); }, 1000);
                    setTimeout(resetBlaster, 2000);
                } 
                else if (w && b) {
                    // 4. MASTER FORM (Keduanya)
                    if (state.userData.unlockedCharacters.includes('master_form')) {
                        // SUKSES
                        if(DOM.blasterComboText) DOM.blasterComboText.textContent = "OVERLOAD...!!";
                        
                        // Efek Getar dan Cahaya Ekstrem
                        const blasterContainer = document.getElementById('blaster-container');
                        if (blasterContainer) blasterContainer.classList.add('blaster-shake');
                        if (DOM.btnWhite) DOM.btnWhite.classList.add('critical-overload');
                        if (DOM.btnBlack) DOM.btnBlack.classList.add('critical-overload');

                        // Suara Henshin Master
                        setTimeout(() => {
                            if(DOM.sfxHenshinMaster) { 
                                DOM.sfxHenshinMaster.currentTime = 0; 
                                DOM.sfxHenshinMaster.play().catch(e=>{}); 
                            }
                        }, 300);
                        
                        // Efek Visual Meledak di Layar
                        setTimeout(() => {
                            // Bola Aura
                            const energySphere = document.createElement('div');
                            energySphere.className = 'henshin-aura-expand';
                            document.body.appendChild(energySphere);
                            
                            // Flash Layar Putih Total
                            const flash = document.createElement('div');
                            flash.style.cssText = "position:fixed;top:0;left:0;width:100%;height:100%;background:white;z-index:9999;opacity:1;transition:opacity 0.8s;";
                            document.body.appendChild(flash);
                            
                            // Ganti 3D Model jadi Master Form (saat layar ketutup putih)
                            setTimeout(() => { switchCharacter(CONFIG.CHARACTERS.find(c => c.id === 'master_form')); }, 200);

                            // Bersihkan Layar dari Efek
                            setTimeout(() => flash.style.opacity = '0', 300);
                            setTimeout(() => { flash.remove(); energySphere.remove(); }, 1200);

                            // Matikan senjata
                            resetBlaster();
                        }, 800); // Waktu jeda sebelum ledakan (sesuaikan dengan audionya)

                    } else {
                        // ERROR (Belum Beli di Garasi)
                        if(DOM.blasterComboText) DOM.blasterComboText.textContent = "ERROR: LOCKED";
                        setTimeout(() => {
                            if(DOM.sfxError) { 
                                DOM.sfxError.currentTime = 0; 
                                DOM.sfxError.play().catch(e=>{}); 
                            }
                        }, 300);
                        setTimeout(resetBlaster, 2000);
                    }
        ...
        // Update Text Status Layar (Kecuali pas trigger)
        if (action !== 'trigger') {
            if (state.blasterWhiteActive && state.blasterBlackActive) {
                if(DOM.blasterComboText) DOM.blasterComboText.textContent = "READY: MASTER";
            } else if (state.blasterWhiteActive) {
                if(DOM.blasterComboText) DOM.blasterComboText.textContent = "READY: FINISHER";
            } else if (state.blasterBlackActive) {
                if(DOM.blasterComboText) DOM.blasterComboText.textContent = "READY: HENSHIN";
            }
        }
    };
