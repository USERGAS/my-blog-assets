            ...
            if (!w && !b) {
                ...
                if (w && !b) {
                    ...
                    if (state.userData.unlockedCharacters.includes('master_form')) {
                        DOM.blasterComboText.textContent = "OVERLOAD...!!";
                        
                        // [EFEK DINAMIS] Senjata bergetar hebat & Lampu meledak-ledak
                        const blasterContainer = document.getElementById('blaster-container');
                        blasterContainer.classList.add('blaster-shake');
                        DOM.btnWhite.classList.add('critical-overload');
                        DOM.btnBlack.classList.add('critical-overload');

                        // Suara Master Form (delay sesuai tring)
                        setTimeout(() => {
                            if(DOM.sfxHenshinMaster) { DOM.sfxHenshinMaster.currentTime=0; DOM.sfxHenshinMaster.play().catch(e=>{}); }
                        }, 300);
                        
                        // [EFEK DINAMIS] Bola energi meluas menutupi layar setelah 0.8 detik
                        setTimeout(() => {
                            ...
                            // Ganti model saat layar tertutup putih
                            setTimeout(() => { switchCharacter(CONFIG.CHARACTERS.find(c => c.id === 'master_form')); }, 200);

                            // Bersihkan efek flash
                            setTimeout(() => flash.style.opacity = '0', 300);
                            setTimeout(() => { flash.remove(); energySphere.remove(); }, 1500);

                            // Bersihkan efek shake & glow overload
                            blasterContainer.classList.remove('blaster-shake');
                            resetBlaster();
                        }, 800); // Waktu energi meledak
