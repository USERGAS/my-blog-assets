...
// ================== LOGIKA BRANKAS & BATTLE (SAFE GAME & RPG) ==================
    ...
    const dealDamageToMonster = (e, t) => {
        if (state.isGameTurn !== "none") {
            playAnimation(DOM.battleModelViewer, t, "PunchC");
            spawnMasterAura(); 
            DOM.sfxAttack?.play();
            triggerVibration(50);
            
            state.currentMonsterHP -= e;
            if (state.currentMonsterHP < 0) state.currentMonsterHP = 0;
            
            DOM.battleMonsterImg.classList.add("flash-red");
            setTimeout(() => {
                DOM.sfxMonsterHit?.play();
                DOM.battleMonsterImg.classList.remove("flash-red");
            }, 150);
            
            state.superMeter += 3.5 * e;
    ...
    const dealDamageToPlayer = e => {
        if (state.isGameTurn !== "none") {
            let t = e;
            addBattleLog(`Monster menyerang dengan kekuatan ${e}!`);
            
            if (state.isPlayerDodging) {
                t = 0;
                addBattleLog("Kamu berhasil menghindar! Tidak ada kerusakan!");
            } else {
    ...
    const showBattleResult = e => {
        DOM.battleResultText.textContent = e ? "MENANG" : "KALAH";
        DOM.battleResultText.className = "battle-result-text";
        DOM.battleResultText.classList.add(e ? "win-text" : "lose-text");
        DOM.battleResultOverlay.style.display = "flex";
        setTimeout(() => endBattle(e), 2000);
    };
    ...
    const switchTurn = e => {
        ...
        if (state.isGameTurn === "enemy") {
            ...
            setTimeout(() => {
                if (state.isGameTurn !== "none") {
                    if (Math.random() < 0.9) {
                        dealDamageToPlayer(Math.floor(11 * Math.random()) + 15);
                    } else {
                        addBattleLog("Monster terlihat waspada dan menjaga pertahanannya.");
                        switchTurn("player");
                    }
                }
            }, 1500);
        } else {
            addBattleLog("Giliranmu! Pilih aksimu selanjutnya.");
        }
    };

    const addBattleLog = e => {
        const t = document.createElement("p");
        t.textContent = `> ${e}`;
        DOM.battleLogArea.appendChild(t);
        DOM.battleLogArea.scrollTop = DOM.battleLogArea.scrollHeight;
    };
...
// ============================================================
    ...
    // --- PROSES GANTI KARAKTER (V18: Instant Load) ---
    const switchCharacter = (character) => {
        DOM.hubPopup.style.display = 'none';
        
        // 1. Cek ID: Kalau karakter sama, gak usah ngapa-ngapain
        if (state.userData.activeCharacter === character.id) {
            applyCharacterMode(character.id);
            return;
        }

        // 2. Cek Cache: Apakah model ini sudah pernah didownload?
        if (skinCache[character.id]) {
            console.log("Memuat dari Cache RAM... Instan!");
            
            // Trik: Kosongkan src sejenak biar browser 'sadar' ada perubahan
            DOM.modelViewer.src = '';
            
            // Ambil URL Blob dari ingatan (Cache)
            ...
            // Update Tampilan (Parameter true = Jangan timpa model lagi)
            applyCharacterMode(character.id, true); 
            
            // Notif Chat
            addMessageToChat('bot', `⚡ Kostum dipasang (Instan): **${character.name}**!`);
            
            // Paksa Animasi Jalan
            setTimeout(() => playAnimation(DOM.modelViewer, CONFIG.ANIMATION_NAME_IDLE), 100);
            return; // STOP DI SINI (Gak perlu download lagi)
        }

        // 3. Kalau belum ada di Cache, BARU DOWNLOAD (XHR)
        ...
        xhr.responseType = "blob"; // Download sebagai data mentah
        ...
        xhr.onload = () => {
            if (xhr.status === 200) {
                ...
                // PENTING: Simpan ke Cache biar nanti bisa dipakai lagi
                ...
                setTimeout(() => playAnimation(DOM.modelViewer, CONFIG.ANIMATION_NAME_IDLE), 100);
                addMessageToChat('bot', `🔄 Kostum berhasil diganti: **${character.name}**!`);
        ...
        const handleError = () => {
            DOM.skinLoadingOverlay.style.display = 'none';
            alert("Gagal mengunduh skin. Cek koneksi internet.");
            // Fallback ke link biasa kalau download gagal
...
// Fungsi Membuat Percikan Api & Layar Error
    const createErrorSparks = () => {
        ...
        // Fungsi Transisi Master Form (SHATTER & ASSEMBLE)
    const playShatterTransition = (targetCharId) => {
        const targetNode = document.fullscreenElement || document.body;
        ...
        // Langsung sembunyikan UI Senjata biar seolah-olah dia yang meledak
        DOM.blasterOverlay.style.display = 'none'; 

        // Buat 45 Serpihan Kaca (Berdasarkan warna senjata: Putih, Hitam, Merah, Perak)
        const shardColors = ['#ffffff', '#111111', '#e74c3c', '#bdc3c7'];
        
        for(let i=0; i<45; i++) {
            const shard = document.createElement('div');
            shard.className = 'shatter-shard';
            
            // Pilih warna acak dari palet senjata
            shard.style.background = shardColors[Math.floor(Math.random() * shardColors.length)];
            
            // Bikin bentuk serpihan acak (Segitiga/Trapesium tidak beraturan)
            ...
            // Hitung arah ledakan melingkar (360 derajat)
            const angle = Math.random() * Math.PI * 2;
            const distance = 150 + Math.random() * 250; // Jarak melayang (150px - 400px)
            
            const tx = Math.cos(angle) * distance + 'px';
            const ty = Math.sin(angle) * distance + 'px';
            const rot1 = (Math.random() - 0.5) * 720 + 'deg'; // Putaran saat meledak
            const rot2 = (Math.random() - 0.5) * 1440 + 'deg'; // Putaran gila saat tersedot masuk
            
            shard.style.setProperty('--tx', tx);
            shard.style.setProperty('--ty', ty);
            shard.style.setProperty('--rot1', rot1);
            shard.style.setProperty('--rot2', rot2);
        ...
        // TIMING SINKRONISASI:
        // Di 1600ms (1.6 detik), pecahan masuk ke tengah dan layar nge-flash paling putih.
        // Saat itulah kita swap modelnya!
        setTimeout(() => {
            switchCharacter(CONFIG.CHARACTERS.find(c => c.id === targetCharId));
        }, 1600);

        // Hapus elemen setelah animasi kelar (2.5 detik)
        setTimeout(() => {
