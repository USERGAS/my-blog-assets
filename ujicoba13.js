...
// ================== GAME RUNNER (SWIPE + VISUAL FX) ==================
    ...
    const startRunnerGame = () => {
        ...
        state.runnerCoins = 0;
        state.runnerLives = 3;
        state.runnerSpeed = 1800; // Sedikit lebih cepat
    ...
    const attachModelToRunner = () => {
        ...
        // SETTING MODEL BIAR LARI & TRANSPARAN
        DOM.modelViewer.style.visibility = "visible";
        DOM.modelViewer.style.backgroundColor = "transparent"; 
        
        // Kamera Belakang
        DOM.modelViewer.cameraOrbit = "180deg 80deg 2.5m"; 
        DOM.modelViewer.fieldOfView = "30deg";
        DOM.modelViewer.removeAttribute('camera-controls'); 
        
        // [FIX ANIMASI] Paksa TimeScale & Play berulang
        DOM.modelViewer.timeScale = 1.5; // Lari 1.5x lebih cepat
        DOM.modelViewer.animationName = "Run"; 
        DOM.modelViewer.play();
        
        // Double check play state setelah delay kecil (mengatasi bug 'diam')
        setTimeout(() => {
            if(state.isRunnerActive) {
                DOM.modelViewer.animationName = "Run";
                DOM.modelViewer.play();
        ...
        const spawnObject = () => {
            ...
            setTimeout(() => {
                ...
                if (state.runnerLane === lane) {
                    // Cek apakah objek belum diambil (cegah double trigger)
            ...
            }, state.runnerSpeed * 0.80); // Cek di 80% perjalanan
    ...
    const collectCoinRunner = (obj) => {
        // [VISUAL FX] Tambah kelas collected untuk animasi CSS pop-up
        ...
        if(state.runnerCoins >= CONFIG.COINS_PER_RUN) {
            setTimeout(() => endRunnerGame(true), 500); // Delay dikit biar efek koin selesai
        }
    };

    const hitObstacleRunner = (obj) => {
        obj.classList.add('hit'); // Efek visual obstacle kena
