...
// --- 1. MENAMPILKAN ID SAAT BUKA TAB AKUN ---
    const updateAccountTab = () => {
        const userId = localStorage.getItem('kaitouVisitorId');
        const inputDisplay = document.getElementById('user-recovery-code');
        if(inputDisplay && userId) {
            inputDisplay.value = userId;
        }
    };

    // --- 2. LOGIKA LOAD AKUN (PULIHKAN DATA) ---
    const restoreAccount = async () => {
        const inputField = document.getElementById('input-recovery-code');
        ...
        // Konfirmasi keamanan
        if (!confirm("PERINGATAN: Memuat akun baru akan MENIMPA data saat ini. Lanjutkan?")) return;

        const btn = document.getElementById('load-account-btn');
        ...
        try {
            ...
            if (data.status === 'success') {
                // UPDATE LOCAL STORAGE DENGAN DATA DARI SERVER
                const restoredData = {
                    username: data.username,
                    coins: parseInt(data.coins) || 0,
                    dayStreak: parseInt(data.streak) || 1,
                    // Parse JSON string array jika data unlockedCharacters berbentuk string
                ...
                alert(`Berhasil! Selamat datang kembali, ${data.username}!`);
                
                // Refresh Halaman untuk menerapkan semua perubahan
                location.reload(); 
            } else {
                alert("Gagal: Kode pemulihan tidak ditemukan di server.");
            }
        } catch (error) {
            console.error(error);
...
// LISTENER TAB AKUN
        ...
        if(copyBtn) {
            copyBtn.addEventListener('click', () => {
                const code = document.getElementById('user-recovery-code');
                code.select();
                navigator.clipboard.writeText(code.value);
                alert("Kode disalin! Simpan di tempat aman.");
            });
        }

        const loadBtn = document.getElementById('load-account-btn');
        if(loadBtn) {
            loadBtn.addEventListener('click', restoreAccount);
        }
    };
    };
