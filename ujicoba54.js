...
// ================== LOGIKA BLASTER (HENSHIN) - FIXED AUDIO ==================
    ...
    // Fungsi untuk matikan semua suara standby biar gak dobel
    ...
    // Fungsi untuk memutar suara standby yang TEPAT sesuai kombinasi
    const evaluateStandbySound = () => {
        const w = state.blasterWhiteActive;
        const b = state.blasterBlackActive;

        stopAllStandby(); // Matikan yang lama dulu

        if (w && b) {
            // Keduanya Aktif -> STANDBY MASTER
            if(DOM.sfxStandbyMaster) DOM.sfxStandbyMaster.play().catch(e=>{});
        } else if (w && !b) {
            // Putih Saja
            if(DOM.sfxStandbyWhite) DOM.sfxStandbyWhite.play().catch(e=>{});
        } else if (!w && b) {
            // Hitam Saja
            if(DOM.sfxStandbyBlack) DOM.sfxStandbyBlack.play().catch(e=>{});
        }
    };
...
const handleBlasterAction = (action) => {
        // HAPUS playTring() dari sini (kita pindah ke dalam kondisi masing-masing)

        if (action === 'white') {
            playTring(); // Bunyi Tring saat tekan putih
            ...
            playTring(); // Bunyi Tring saat tekan hitam
            ...
            stopAllStandby(); // Matikan suara standby saat ditembak
            ...
            if (!w && !b) {
                // ==========================================
                // 1. NORMAL BLAST (TIDAK ADA TRING)
                // ==========================================
                ...
                // ==========================================
                // JIKA ADA COMBO (Maka bunyi Tring dulu)
                // ==========================================
                playTring();

                // Kita kasih delay 300ms (0.3 detik) biar suara Tring kedengaran 
                // sebelum suara ledakan/henshin menyusul (Seperti mainan DX asli)
            ...
            // Kembalikan ke awal setelah nembak
...
// ============================================================
    // MIXER AUDIO (SETTING VOLUME & SEAMLESS LOOP)
    // ============================================================
    const setupAudioMixer = () => {
        
        // 1. ATUR VOLUME MASING-MASING SUARA (Skala 0.0 sampai 1.0)
        // 1.0 = Sangat Keras, 0.5 = Sedang, 0.1 = Sangat Pelan
        
        if (DOM.sfxTring) DOM.sfxTring.volume = 1.0;
        
        // Suara Standby kita buat agak pelan biar gak nutupin suara lain
        ...
        // Suara Efek Tembakan & Ledakan kita buat Maksimal
        ...
        // 2. LOGIKA SEAMLESS LOOP (Memotong Jeda Kosong MP3)
        const makeSeamless = (audio) => {
            if (!audio) return;
            // Kita dengerin posisi detiknya
            audio.addEventListener('timeupdate', function() {
                // Jika lagu sudah sampai 0.15 detik SEBELUM lagu habis, paksa balik ke 0
                // (Kamu bisa ubah angka 0.15 jadi 0.2 kalau masih kerasa ada jedanya)
        ...
        // Terapkan efek seamless hanya ke audio standby
        makeSeamless(DOM.sfxStandbyWhite);
        makeSeamless(DOM.sfxStandbyBlack);
        makeSeamless(DOM.sfxStandbyMaster);
    };
