...
// ============================================================
    // LOGIKA BLASTER: RESET & VISUAL EFFECTS
    // ============================================================
    
    // Fungsi untuk mengembalikan senjata ke kondisi mati
    const resetBlaster = () => {
        ...
        // MATIKAN SEMUA CAHAYA VISUAL
        if(DOM.btnWhite) DOM.btnWhite.classList.remove('glow-white-active');
        if(DOM.btnBlack) DOM.btnBlack.classList.remove('glow-black-active');
        if(DOM.btnTrigger) DOM.btnTrigger.classList.remove('trigger-pulled');
    };
    ...
    const handleBlasterAction = (action) => {
        if (action === 'white') {
            ...
            if (!state.blasterWhiteActive) {
                ...
                DOM.btnWhite.classList.add('glow-white-active');
            }
        } 
            ...
            if (!state.blasterBlackActive) {
                ...
                // NYALAKAN CAHAYA MERAH (Di area Hitam)
                DOM.btnBlack.classList.add('glow-black-active');
            }
        } 
        else if (action === 'trigger') {
            stopAllStandby(); 
            
            // Efek visual pelatuk ditekan kilat
            DOM.btnTrigger.classList.add('trigger-pulled');

            const w = state.blasterWhiteActive;
            const b = state.blasterBlackActive;

            // (LOGIKA SUARA & HENSHIN SAMA SEPERTI SEBELUMNYA)
            if (!w && !b) {
                ...
                if (w && !b) {
                    ...
                    if (state.userData.unlockedCharacters.includes('master_form')) {
                        ...
                        // KEDUA CAHAYA BERKEDIP CEPAT SEBELUM MELEDAK
                        DOM.btnWhite.style.animationDuration = "0.1s";
                        DOM.btnBlack.style.animationDuration = "0.1s";
                        ...
                        setTimeout(() => { switchCharacter(CONFIG.CHARACTERS.find(c => c.id === 'master_form')); }, 1500);
                    } else {
            ...
            setTimeout(() => {
                resetBlaster();
                // Kembalikan speed animasi kalau tadi dirubah pas Henshin Master
                DOM.btnWhite.style.animationDuration = "";
                DOM.btnBlack.style.animationDuration = "";
            }, 2000);
        }
