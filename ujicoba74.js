    ...
    const CONFIG = {
        ...
        CHARACTERS: [
            ...
            { 
                ...
                icon: '🌟', 
                modelSrc: 'https://cdn.jsdelivr.net/gh/USERGAS/my-blog-assets/MasterFormHD.glb', 
                price: 1000,
...
// --- MASTER SWITCH (FIXED: NAMA HEADER & CHAT MASTER FORM) ---
    const applyCharacterMode = (charId, skipModelUpdate = false) => {
        const char = CONFIG.CHARACTERS.find(c => c.id === charId);
        if (!char) return;

        // 1. Ubah Judul Header (Dengan Penyingkatan)
        if (DOM.chatHeaderTitle) {
            let displayName = char.name;
            // Singkat nama Kamen Rider
            displayName = displayName.replace("Kamen Rider", "K.R");
            // Singkat Master Form biar muat di header
        ...
        // [FIX] Kaitou biasa DAN Master Form tetep bisa ngechat!
        if (charId !== 'kaitou' && charId !== 'master_form') {
            // === MODE GAME (Skin Orang Lain / Kamen Rider) ===
       ...
       if (char.cameraTarget) {
            DOM.modelViewer.cameraTarget = char.cameraTarget;
        }

        // 5. Update Model Battle
        if (DOM.battleModelViewer) {
            const mainSrc = DOM.modelViewer.src; 
            if (mainSrc) {
                DOM.battleModelViewer.src = mainSrc;
            }
         if (char.cameraTarget) {
                DOM.battleModelViewer.cameraTarget = char.cameraTarget;
            }
        }
         manageMasterIdleAura();
    };
...
// ============================================================
    // GENERATOR IDLE AURA (Petir yang keluar otomatis tanpa klik)
    // ============================================================
    let masterIdleAuraTimer = null; // Penyimpan interval

    const manageMasterIdleAura = () => {
        // Matikan timer lama kalau ada
        if (masterIdleAuraTimer) clearInterval(masterIdleAuraTimer);
        
        // Nyalakan HANYA jika sedang pakai Master Form
        if (state.userData.activeCharacter === 'master_form') {
            
            // Loop setiap 400 - 800 ms
            masterIdleAuraTimer = setInterval(() => {
                
                // JANGAN keluar aura kalau lagi main game lain (Biar gak ganggu layar)
                if (DOM.battleOverlay.style.display === 'flex' || state.isRunnerActive || state.isSafeGameActive) return;
                
                const container = document.getElementById('kaitou-chat-body-container');
                if (!container) return;

                // Keluarin 1 atau 2 petir kecil secara acak
                const boltCount = Math.random() > 0.5 ? 2 : 1;

                for (let i = 0; i < boltCount; i++) {
                    const bolt = document.createElement('div');
                    bolt.className = 'idle-lightning-bolt';
                    
                    // Muncul random di sekitar badan karakter
                    const topPos = 20 + Math.random() * 60; // 20% - 80% layar
                    const leftPos = 35 + Math.random() * 30; // 35% - 65% (Fokus di tengah)
                    const height = 30 + Math.random() * 60; // Petir lebih pendek (30px - 90px)
                    const rotation = Math.random() * 360;
                    ...
                    // Hapus setelah 200ms
                    setTimeout(() => { if (bolt.parentNode) bolt.remove(); }, 200);
                }
            }, 600); // 600 milidetik
        }
    };
