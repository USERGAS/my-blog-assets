...
// Fungsi Transisi Layar Henshin (MATERIALIZE BOTTOM-TO-TOP)
    const playHenshinTransition = (targetCharId, glowColor) => {
        
        // 1. Siapkan Wadah Materialize
        const container = document.createElement('div');
        container.className = 'materialize-container';

        // 2. Buat Tirai & Laser
        const curtain = document.createElement('div');
        curtain.className = 'materialize-curtain';
        curtain.style.setProperty('--glow-color', glowColor); // Set warna laser (Biru/Emas)

        const laser = document.createElement('div');
        laser.className = 'materialize-laser';

        curtain.appendChild(laser);
        container.appendChild(curtain);
        document.body.appendChild(container);

        // 3. Efek Partikel Data Digital yang berjatuhan saat laser naik
        const createParticles = setInterval(() => {
            for(let i=0; i<3; i++) {
                const particle = document.createElement('div');
                particle.className = 'materialize-particle';
                particle.style.left = (Math.random() * 100) + '%';
                // Tinggi partikel random biar natural
                particle.style.height = (10 + Math.random() * 40) + 'px'; 
        ...
        // 4. Timing Sinkronisasi GANTI MODEL
        // Saat tirai mulai naik (sekitar 300ms setelah muncul)
        setTimeout(() => {
            // Tutup overlay senjata
            DOM.blasterOverlay.style.display = 'none'; 
            
            // Ganti 3D Model (Di balik layar yang sedang terbuka)
            switchCharacter(CONFIG.CHARACTERS.find(c => c.id === targetCharId));
        }, 300);

        // 5. Bersihkan semua efek setelah selesai (1.5 detik)
        setTimeout(() => {
            clearInterval(createParticles);
            container.remove();
        }, 1500);
    };
        ...
        if (action === 'white') {
            ...
            if (!w && !b) {
                ...
                if (w && !b) {
                    ...
                    setTimeout(() => { 
                        playHenshinTransition('kaitou', ['#ffffff', '#3498db']); 
                    }, 1000);
                } 
                else if (w && b) {
                    // MASTER FORM
                    if (state.userData.unlockedCharacters.includes('master_form')) {
                        ...
                        setTimeout(() => { 
                            playHenshinTransition('master_form', ['#ffffff', '#f1c40f']); 
                        }, 1200);
