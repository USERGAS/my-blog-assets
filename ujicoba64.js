    ...
    const handleBlasterAction = (action) => {
        if (action === 'white') {
            ...
            if (!state.blasterBlackActive) {
                ...
                // NYALAKAN LAMPU MERAH 💡 (Mata Hitam)
            ...
            // EFEK KILAT PELATUK
            if (DOM.btnTrigger) {
                DOM.btnTrigger.classList.remove('trigger-flash'); // Reset dulu
                void DOM.btnTrigger.offsetWidth; // Trigger reflow browser
            ...
            if (!w && !b) {
                ...
                if (w && !b) {
                    ...
                    setTimeout(() => { 
                        DOM.blasterOverlay.style.display = 'none'; // Tutup layar senjata
                        switchCharacter(CONFIG.CHARACTERS.find(c => c.id === 'kaitou')); 
                    }, 1000);
                } 
                else if (w && b) {
                    // MASTER FORM
                    if (state.userData.unlockedCharacters.includes('master_form')) {
                        ...
                        if(blasterImg) blasterImg.classList.add('weapon-shake'); // Senjata bergetar hebat

                        setTimeout(() => {
                            if(DOM.sfxHenshinMaster) { DOM.sfxHenshinMaster.currentTime=0; DOM.sfxHenshinMaster.play().catch(e=>{}); }
                        }, 300);
                        
                        // Efek Flash Putih Layar
                        const flash = document.createElement('div');
                        flash.style.cssText = "position:fixed;top:0;left:0;width:100%;height:100%;background:white;z-index:9999;opacity:1;transition:opacity 1s;";
                        ...
                        setTimeout(() => { 
                            DOM.blasterOverlay.style.display = 'none'; // Tutup layar senjata
                            switchCharacter(CONFIG.CHARACTERS.find(c => c.id === 'master_form')); 
                        }, 1500);
                    } else {
                        // ERROR LISENSI
            ...
            // Kembalikan ke awal setelah nembak (Lampu mati, suara mati)
