...
// Variabel global untuk menyimpan URL Blob sementara (Taruh di atas switchCharacter)
    let currentBlobUrl = null;

    // --- PROSES GANTI KARAKTER (FIXED: Reset Source & Memory Cleanup) ---
    const switchCharacter = (character) => {
        DOM.hubPopup.style.display = 'none';
        
        // TAMPILKAN LAYAR LOADING DULUAN
        ...
        // [FIX PENTING] Kosongkan model saat ini agar browser tidak bingung
        DOM.modelViewer.src = ''; 
        ...
        xhr.onload = () => {
            if (xhr.status === 200) {
                // [FIX MEMORY] Hapus URL blob lama dari memori jika ada
                if (currentBlobUrl) {
                ...
                // Masukkan ke Model Viewer (Paksa Update)
                DOM.modelViewer.src = currentBlobUrl;
                
                // Simpan State
                state.userData.activeCharacter = character.id;
                saveUserData();
                
                // Terapkan Mode (Skip update model di dalam fungsi ini karena sudah kita set di atas)
                applyCharacterMode(character.id, true);
                
                // Paksa Animasi Idle
                // Kita kasih timeout super kecil biar model sempat 'sadar' kalau dia sudah load
                setTimeout(() => {
                    playAnimation(DOM.modelViewer, CONFIG.ANIMATION_NAME_IDLE);
                }, 100);
        ...
        const handleError = () => {
            DOM.skinLoadingOverlay.style.display = 'none';
            // Kembalikan model ke settingan config kalau gagal
            DOM.modelViewer.src = character.modelSrc; 
            alert("Gagal mengunduh skin. Menggunakan koneksi standar.");
        };
