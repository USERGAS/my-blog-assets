document.addEventListener("DOMContentLoaded", () => {
    ...
    const CONFIG = {
        ...
        SHOP_ITEMS: [{ id: 'access_code', name: 'Kode Akses 2 Hari Link Tanpa Iklan', icon: '🔑', cost: 900, discountCost: 100 }],
...
Ada yang mau ditanyakan? Atau mau langsung mulai berburu misi?`}]} ]; };
    ...
    const switchTab = (e) => {
        ...
        DOM.hubTabs.querySelector(`.hub-tab-btn[data-tab="${e}"]`).classList.add("active");
        DOM.hubPopupBox.querySelectorAll(".hub-content").forEach(t => t.classList.remove("active"));
        document.getElementById(`hub-content-${e}`).classList.add("active");
    };
    ...
    const populateShopList = () => {
        ...
        CONFIG.SHOP_ITEMS.forEach(item => {
            ...
            if (state.userData.coins < currentPrice) {
                li.querySelector(".redeem-btn").disabled = true;
            }
    ...
    const populateHistoryList = () => {
        ...
        0 === e.length ? DOM.historyList.innerHTML = '<p id="no-history-msg">Anda belum pernah menukarkan kode.</p>' : e.reverse().forEach(e => {
            const t = document.createElement("li");
            const n = new Date(e.timestamp).toLocaleString("id-ID", { day: "numeric", month: "short", year: "numeric", hour: "2-digit", minute: "2-digit" });
            t.innerHTML = `<div class="item-details"><span class="item-name">Kode: <code class="item-code">${e.code}</code></span><span class="item-timestamp">Ditukar pada: ${n}</span></div><button class="copy-btn" data-code="${e.code}">Salin</button>`;
    ...
    // --- LOGIKA PENENTUAN HARGA BARU DIMULAI DI SINI ---
    const now = new Date();
    const discountEndDate = CONFIG.DISCOUNT_END_DATE ? new Date(CONFIG.DISCOUNT_END_DATE) : new Date(0);
    let currentPrice = item.cost; // Harga default adalah harga normal

    // Cek apakah diskon masih aktif
    if (now <= discountEndDate && item.discountCost) {
        currentPrice = item.discountCost; // Jika ya, gunakan harga diskon
    }
    // --- LOGIKA PENENTUAN HARGA SELESAI ---

    // Gunakan 'currentPrice' yang sudah benar di dalam pesan konfirmasi
