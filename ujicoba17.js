...
// --- 1. HANDLE TOMBOL GARASI (BELI & GUNAKAN) ---
    ...
    // --- 3. MASTER SWITCH (UBAH TAMPILAN WIDGET) ---
    const applyCharacterMode = (charId) => {
        ...
        // 2. Logic Input Chat & Game Mode
        if (charId !== 'kaitou') {
            // JIKA RIDER (Bukan Kaitou)
            state.isGameMode = true; 
            if(DOM.chatInputArea) DOM.chatInputArea.style.display = 'none'; // Sembunyikan Input
            if(DOM.animationToggleBtn) DOM.animationToggleBtn.style.display = 'none'; // Sembunyikan tombol animasi manual
        } else {
            // JIKA KAITOU-KUN (Default)
            state.isGameMode = false;
            if(DOM.chatInputArea) DOM.chatInputArea.style.display = 'flex'; // Munculkan Input
            if(DOM.animationToggleBtn) DOM.animationToggleBtn.style.display = 'block';
        }
        
        // Pastikan src model sesuai (untuk inisialisasi awal)
        const currentSrc = DOM.modelViewer.getAttribute('src');
...
// ================== LOGIKA GAME: BATTLE (RED DOT) ==================
    ...
    // ================== LOGIKA GAME: BRANKAS (YELLOW DOT) ==================
    const startSafeGame = () => {
        ...
        for(let i=0; i<3; i++) state.safeCombination.push(Math.floor(Math.random() * 360));
        
        const knob = DOM.safeDial.querySelector('.dial-knob');
        if(knob) { knob.style.backgroundColor = ''; knob.style.boxShadow = ''; knob.style.borderColor = ''; }
    ...
    const checkSafeSweetSpot = () => {
        ...
        if (inZone) {
            if(knob) {
                knob.style.backgroundColor = '#f1c40f';
            ...
            if (!state.safeUnlockTimer) {
                if(DOM.sfxSafeClick) { DOM.sfxSafeClick.currentTime = 0; DOM.sfxSafeClick.play().catch(e => {}); }
                triggerVibration(50);
            ...
            if(knob) { knob.style.backgroundColor = ''; knob.style.boxShadow = ''; knob.style.borderColor = ''; }
            DOM.safeDial.classList.remove('active-glow');
            if (state.safeUnlockTimer) { clearTimeout(state.safeUnlockTimer); state.safeUnlockTimer = null; }
        }
    };
    ...
    const unlockSafeLevel = () => {
        ...
        const knob = DOM.safeDial.querySelector('.dial-knob');
        if(knob) { knob.style.backgroundColor = ''; knob.style.boxShadow = ''; }
        DOM.safeDial.classList.remove('active-glow');
    ...
    const endSafeGameWin = () => {
        ...
        if (DOM.chatWindow.classList.contains("visible")) { DOM.modelViewer.style.visibility = "visible"; }
    ...
    const handleDialInput = (e, isEnd = false) => {
        if (isEnd) { state.isDraggingDial = false; return; }
        ...
        if (Math.abs(degrees - state.lastTickAngle) > 15) {
            if(DOM.sfxSafeTick) { DOM.sfxSafeTick.currentTime = 0; DOM.sfxSafeTick.play().catch(e => {}); }
    ...
    const openUserHub = (tab) => { 
        ...
        populateShopList(); 
        populateHistoryList();
        populateGarageList(); // Pastikan garasi dirender
        switchTab(tab || "shop"); 
    ...
    const populateShopList = () => {
        ...
        CONFIG.SHOP_ITEMS.forEach(item => {
            ...
            if (isDiscountActive) {
                priceHTML = `<span class="item-price" style="color: #4caf50; font-weight: bold;"><del style="color: #999;">${item.cost} Koin</del> ${item.discountCost} Koin (EVENT TERBATAS)</span>`;
            }

            li.innerHTML = `<div class="shop-item-icon">${item.icon}</div><div class="item-details"><span class="item-name">${item.name}</span>${priceHTML}</div><button class="redeem-btn" data-item-id="${item.id}">Tukar</button>`;
            ...
            if (state.userData.coins < finalCost) {
                ...
                btn.disabled = true; btn.style.opacity = "0.5"; btn.innerText = "Koin Kurang";
            }
    ...
    const setupEventListeners = () => {
        ...
        DOM.animationControls.addEventListener("click", (e) => { if (e.target.tagName === 'BUTTON' && e.target.dataset.animation) { const anim = e.target.dataset.animation; playAnimation(DOM.modelViewer, anim); } });
        ...
        DOM.garageGrid.addEventListener('click', (e) => {
            if (e.target.matches('.char-action-btn')) handleGarageAction(e.target);
        });
        ...
        DOM.shopList.addEventListener('click', async (e) => { 
            ...
            if (now <= discountEndDate && item.discountCost) currentPrice = item.discountCost; 
            
            if (state.userData.coins < currentPrice) { alert("Koin kamu tidak cukup!"); return; } 
            
            if (!confirm(`Anda yakin ingin menukarkan ${currentPrice} koin dengan "${item.name}"?`)) return; 
            
            redeemButton.disabled = true; redeemButton.textContent = 'Memproses...'; 
            try { 
                const userId = localStorage.getItem('kaitouVisitorId'); 
                if (!userId) { alert('ID Pengguna tidak ditemukan. Coba refresh halaman.'); redeemButton.disabled = false; return; } 
                const response = await fetch(`${CONFIG.APPS_SCRIPT_URL}?action=redeem_code&userId=${userId}&itemId=${itemId}`); 
                const data = await response.json(); 
                if (data.status === 'success') { 
                    alert(`Penukaran Berhasil! Kode: ${data.code}`); 
                    state.userData.coins = data.newBalance; 
                    saveUserData(); updateCoinDisplay(); 
                    const history = JSON.parse(localStorage.getItem("kaitouRedeemedCodes")) || []; 
                    ...
                    alert(`Gagal Menukar: ${data.message}`); redeemButton.disabled = false; redeemButton.textContent = 'Tukar'; 
                } 
            } catch (error) { 
                alert('Terjadi kesalahan jaringan.'); redeemButton.disabled = false; redeemButton.textContent = 'Tukar'; 
            } 
        });
        ...
        DOM.battleActionButtons.addEventListener("click", e => { 
            if ("player" === state.isGameTurn && e.target.matches(".action-btn")) { 
                ...
                if (action === "punch") dealDamageToMonster(12, "PunchC"); 
                    ...
                    state.isPlayerDodging = true; playAnimation(DOM.battleModelViewer, "Jump1"); addBattleLog("Kamu bersiap untuk menghindar!"); switchTurn("enemy"); 
    ...
    const initialize = () => {
        ...
        const checkDarkMode = () => { const isDark = document.body.classList.contains("dark-mode") || document.documentElement.classList.contains("dark-mode"); DOM.chatbotContainer.classList.toggle("kaitou-dark-mode", isDark); }; 
        ...
        const updateCountdown = () => { const now = new Date(); let nextReset = new Date(); nextReset.setHours(CONFIG.MISSION_RESET_HOUR_LOCAL, 0, 0, 0); if (now > nextReset) nextReset.setDate(nextReset.getDate() + 1); const diff = nextReset - now; const hours = String(Math.floor(diff / 36e5)).padStart(2, "0"); const minutes = String(Math.floor(diff % 36e5 / 6e4)).padStart(2, "0"); const seconds = String(Math.floor(diff % 6e4 / 1e3)).padStart(2, "0"); if (DOM.missionCountdownElem) DOM.missionCountdownElem.textContent = `Reset dalam: ${hours}:${minutes}:${seconds}`; }; updateCountdown(); setInterval(updateCountdown, 1000);
