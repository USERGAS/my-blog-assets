...
Ada yang mau ditanyakan? Atau mau langsung mulai berburu misi?`}]} 
    ...
    const startSafeGame = () => {
        ...
        // Reset Visual Knob & Border
        const knob = DOM.safeDial.querySelector('.dial-knob');
        if(knob) {
    ...
    const checkSafeSweetSpot = () => {
        ...
        // Sweet Spot Zone (10 derajat tolerance)
        const inZone = diff < 10 || (360 - diff) < 10;

        if (inZone) {
            // [VISUAL PAKSA] Ganti Style Langsung biar gak ada alasan gak berubah
            if(knob) {
                knob.style.backgroundColor = '#f1c40f'; // Kuning Emas
                knob.style.boxShadow = '0 0 20px #f1c40f, inset 0 0 10px #fff';
            ...
            if (!state.safeUnlockTimer) {
                ...
                state.safeUnlockTimer = setTimeout(() => {
                    unlockSafeLevel();
                }, 500); // Tahan cuma 0.5 detik
            }
        } else {
            // [RESET VISUAL] Balikin ke asal
            if(knob) {
                knob.style.backgroundColor = ''; 
                knob.style.boxShadow = '';
                knob.style.borderColor = '';
    ...
    const unlockSafeLevel = () => {
        ...
        // Reset visual sementara saat pindah level
        const knob = DOM.safeDial.querySelector('.dial-knob');
        if(knob) {
            knob.style.backgroundColor = ''; 
            knob.style.boxShadow = '';
