...
// Fungsi Membuat Percikan Api & Layar Error (FIXED)
    const createErrorSparks = () => {
        const container = document.getElementById('blaster-container');
        if (!container) return;

        // 1. Munculkan Layar Petir Merah (Tempel ke Body layar, BUKAN ke kotak senjata)
        const lightning = document.createElement('div');
        lightning.className = 'error-lightning-overlay';
        document.body.appendChild(lightning); // <--- INI FIX NYA
        setTimeout(() => lightning.remove(), 1500); 

        // 2. Lempar 20 Percikan Api (Ini tetap di kotak senjata biar pas keluarnya dari pistol)
        for(let i = 0; i < 20; i++) {
            ...
            const tx = (Math.random() - 0.5) * 400 + 'px'; // Jarak lemparan api diperjauh
...
// Fungsi Transisi Layar Henshin (PREMIUM MATERIALIZE)
    const playHenshinTransition = (targetCharId, hexColor, rgbaColor) => {
        ...
        // Suntikkan variabel warna ke CSS
        ...
        // SPANNER KUBUS DATA (Menciptakan ilusi armor sedang dicetak)
        const particleInterval = setInterval(() => {
            // Ciptakan 5-8 kubus setiap 40 milidetik
            const cubeCount = Math.floor(Math.random() * 4) + 5;
            for(let i=0; i < cubeCount; i++) {
                const cube = document.createElement('div');
                cube.className = 'mat-data-cube';
                
                // Ukuran kubus acak (5px - 20px)
                const size = 5 + Math.random() * 15;
                cube.style.width = size + 'px';
                cube.style.height = size + 'px';
                
                // Posisi horizontal acak di sepanjang laser
                cube.style.left = (Math.random() * 100) + 'vw';
                
                // Posisi vertikal tepat di garis laser
                cube.style.bottom = '0px'; 
                
                curtain.appendChild(cube);
                
                // Hapus kubus dari memori setelah animasinya (1.2s) selesai
                setTimeout(() => { if(cube.parentNode) cube.remove() }, 1200);
            }
        }, 40);

        // TIMING SINKRONISASI (Ganti 3D Model saat layar masih gelap)
        // Layar tertutup penuh dari 0ms sampai ~500ms
        setTimeout(() => {
            DOM.blasterOverlay.style.display = 'none'; 
            switchCharacter(CONFIG.CHARACTERS.find(c => c.id === targetCharId));
        }, 400);

        // BERSIH-BERSIH: Hapus seluruh efek setelah 3 detik (sesuai durasi animasi)
        setTimeout(() => {
